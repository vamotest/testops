...
- [Allure Integration](#allure-integration)
  ...
  - [Локальный Allure](#локальный-allure)
- [Allure](#allure)
  - [step](#step)
  - [issue](#issue)
  - [feature](#feature)
...
## Allure Integration
`Allure Framework` — это гибкий и легкий многоязычный инструмент для отчетов о тестировании, который не только показывает 
очень сжатое представление того, что было протестировано в удобной форме веб-отчета, 
но и позволяет всем, кто участвует в процессе разработки, извлекать максимум полезной информации из повседневного 
выполнения тестов.
...
**[⬆ Back to Index](#index)**
### allure-pytest
[allure-pytest](https://pypi.org/project/allure-pytest/) - плагин для тестового фреймворка [pytest](https://github.com/pytest-dev/pytest).
Документация доступна [онлайн](https://docs.qameta.io/allure-report/), также вы можете получить помощь на канале [gitter](https://gitter.im/allure-framework/allure-core).

С точки зрения установка в Poetry:
```shell script
poetry add --dev allure-pytest
```

Затем в `pytest.ini_options` необходимо добавить:
```
[tool.pytest.ini_options]
addopts = "--alluredir=artifacts/allure_results"
```

Иерархия `artifacts` максимально проста. Она включает в себя директорию `allure_results`, 
куда будут непосредственно записывать результаты наших прогонов.
```
artifacts
├── allure_results
└── .gitignore
...
Лучше также добавить файл `artifacts/.gitignore` с содержимым `*`, 
для того чтобы артефакты не попадали в индекс и не мешали локальной разработке
...
**[⬆ Back to Index](#index)**
### CI Artifacts
С точки зрения сохранения тестовых артефактов для [job'ы](https://gitlab.samoletgroup.ru/devops10d/pipelines/tests/-/blob/main/django-unit.yaml#L101) используется блок [artifacts](https://gitlab.samoletgroup.ru/devops10d/pipelines/tests/-/blob/main/django-unit.yaml#L101).
...
В кластере `Стройка` реализовано так:
```yaml
version: '3.9'
...
# tests/constants.py
class Constants:
    JIRA_URL: str = 'https://hd.samoletgroup.ru/browse/{task_id}'
...
```python
poetry add git+https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils@v.1.0.1
```

Релизы смотреть [тут](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/releases), также об изменениях буду писать в канале [Samolet | Тестирование](https://t.me/+bLMxqFVCfwU3MGEy).
...
**[⬆ Back to Index](#index)** 
### allure_utils
Чтобы разработчикам и AQA было чуть удобнее с точки зрения написание автотестов, то был реализован класс [AllureTestUtils](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/allure_utils.py#L26).
...
**[⬆ Back to Index](#index)** 
#### attach_json
[Данный метод](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/allure_utils.py#L34) позволяет сохранять `data: dict` с `indent=2`.
...
**[⬆ Back to Index](#index)** 
#### attach_txt
[Данный метод](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/allure_utils.py#L40) позволяет сохранять любой тип данных `data: Any`.
...
**[⬆ Back to Index](#index)** 
#### attach_html
[Данный метод](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/allure_utils.py#L46) позволяет сохранять некую строку как HTML, если она такой является. В противном случае как обычную строку.
...
**[⬆ Back to Index](#index)** 
### compare_utils
С точки упрощения зрения написание автотестов был реализован класс [CompareUtils](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/compare_utils.py#L13)
...
Так как методы данного [класса](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/compare_utils.py#L13) содержат `assert`'ы, то класс
...
**[⬆ Back to Index](#index)** 
#### compare_str
[Данный метод](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/compare_utils.py#L69) позволяет сравнивать строку с фактическим и ожидаемым результатом.
Если строки идентичны, то наша проверка пройдут успешно. В противном случае мы получим [AssertionError](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/compare_utils.py#L66), с отличиями
...
**[⬆ Back to Index](#index)** 
#### compare_dict
[Данный метод](https://gitlab.samoletgroup.ru/devdept/tests-utilities/tests-utils/-/blob/dev/tests_utils/compare_utils.py#L82) позволяет сравнивать словари с фактическим и ожидаемым результатом.
